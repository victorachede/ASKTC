 ...
 * Animates brand pillars in a minimalist pill.
 */
const StatusTicker = () => {
...
export default function LandingPage() {
  ...
  return (
    <main className="min-h-screen bg-[#050505] text-white antialiased font-sans selection:bg-white selection:text-black">
      ...
      <nav className="sticky top-0 z-50 border-b border-white/5 bg-black/80 backdrop-blur-md">
        <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-8">
            <Link 
              href="/create" 
              className="text-[9px] font-black text-gray-500 hover:text-white uppercase tracking-[0.3em] transition-colors"
            >
              Start Session
            </Link>
            <button 
              onClick={() => router.push('/leader-login')}
              className="px-5 py-2 bg-white text-black text-[10px] font-black uppercase tracking-[0.2em] rounded-sm hover:bg-gray-200 transition-all active:scale-95"
            >
              Sign In
            </button>
          </div>
      ...
      <section className="max-w-5xl mx-auto px-6 pt-32 pb-24 text-center">
        ...
        <p className="text-gray-500 text-sm md:text-lg font-medium max-w-xl mx-auto mb-16 leading-relaxed">
          The simplest way to run Q&A for your events. <br /> 
          No friction, no apps, just conversation.
        </p>

        {/* JOIN INPUT BOX */}
        <div className="max-w-sm mx-auto flex items-center p-1 bg-white/5 border border-white/10 rounded-xl focus-within:border-white/30 transition-all shadow-2xl shadow-black">
          <input 
            type="text" 
            ...
            className="flex-1 bg-transparent px-5 py-4 outline-none font-bold text-sm tracking-[0.4em] uppercase placeholder:text-gray-800"
          />
          <button 
            onClick={handleJoin}
            className="px-8 py-4 bg-white text-black text-[10px] font-black uppercase tracking-widest rounded-lg hover:bg-gray-200 active:scale-95 transition-all"
          >
            Join
          </button>
        </div>
      ...
      <section className="max-w-6xl mx-auto px-6 py-20">
        <div className="grid grid-cols-1 md:grid-cols-12 gap-px bg-white/10 border border-white/10 rounded-sm overflow-hidden">
          ...
          {/* Social Proof */}
          <div className="md:col-span-4 p-12 bg-black flex flex-col justify-between border-l border-white/10">
          ...
          {/* Feature: Sync */}
          <div className="md:col-span-4 p-12 bg-black flex flex-col justify-between border-t border-l border-white/10">
